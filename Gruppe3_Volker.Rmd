...
model_svm <- svm(Umsatz ~ Warengruppe + Wochentag + KielerWoche + Monat + Bewoelkung, train_dataset) #+ Wochentag + KielerWoche + Monat
```
...
pred_new <- predict(model_svm, newData = newData)
```
