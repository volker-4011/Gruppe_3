...
```{r}
    ...
    for (i in as.numeric(row.names(fullData2))){
     
      if(is.na(fullData2$Bewoelkung[i])){

        dayone <- max(fullData2$Datum[i])-1
        daytwo <- max(fullData2$Datum[i])+1

        dayone_value = max(fullData2$Bewoelkung[i-1])
        
        daytwo_value = NA
        j = 1
        while (is.na(daytwo_value)) {   
          daytwo_value = max(fullData2$Bewoelkung[i+j])
          j = j + 1
        } 

        dayone_utc = as.numeric(as.POSIXct(max(fullData2$Datum[i])-1, format="%Y-%m-%d"))
        daytwo_utc = as.numeric(as.POSIXct(max(fullData2$Datum[i])+1, format="%Y-%m-%d"))
        daycurrent_utc = as.numeric(as.POSIXct(max(fullData2$Datum[i]), format="%Y-%m-%d"))

        daycurrent_value = dayone_value + ((daytwo_value - dayone_value) * ((daycurrent_utc - dayone_utc)/(daytwo_utc - dayone_utc)))
        
        
        fullData2$Bewoelkung[i] <- round(daycurrent_value)
...
forecastData$Bewoelkung <- Umsatz_predicted
...
colnamesPred <- c("Datum","Bewoelkung")
#colnamesPred
#typeof(colnamesPred)
fullData2 <- fullData2[colnamesPred]
forecastData <- forecastData[colnamesPred]




#colnamesPred <- colnames(fullData2)
#colnamesPred
#typeof(colnamesPred)
#forecastData <- forecastData[colnamesPred]

selectedRows <- (fullData2$Datum %in% forecastData$Datum)

fullData2 <- fullData2[selectedRows,]

fullData2 <- rename(fullData2, Bewoelkung_interpolation = Bewoelkung)
forecastData <- rename(forecastData, Bewoelkung_forecast = Bewoelkung)

#    unlink("testforecast.csv")
#    writecsvData <- forecastData
#    writecsvData <- cbind(ID = 1:nrow(writecsvData), writecsvData)

#    write.csv(writecsvData,"testforecast.csv", append = FALSE, quote = TRUE, sep = ",",
...
#    unlink("testinterpol.csv")
#    writecsvData <- fullData2
#    writecsvData <- cbind(ID = 1:nrow(writecsvData), writecsvData)

#    write.csv(writecsvData,"testinterpol.csv", append = FALSE, quote = TRUE, sep = ",",
##              eol = "\n", na = "NA", dec = ".", row.names = FALSE,
#              col.names = TRUE, qmethod = c("escape", "double"),
#              fileEncoding = "")

#forecastData <- read_csv("testforecast.csv")
#fullData2 <- read_csv("testinterpol.csv")


vergleich <- merge(fullData2,forecastData,by="Datum")
...
#str(fullData2)
#forecastData$Bewoelkung_forecast <- as.numeric(forecastData$Bewoelkung_forecast)
#str(forecastData)
#vergleich <- merge(fullData2,forecastData,by="Datum")
